...
export default function ClientBuilder(){
  ...
  useEffect(()=>{
    try{
      const p = typeof window !== 'undefined' ? new URLSearchParams(window.location.search).get('pkg') : null
      if(p) setPkg(p)
    }catch(e){}
  },[])

  const [step, setStep] = useState(0)
  const [form, setForm] = useState<any>({ siteName:'', company:'', goals:'', theme:'Modern', colors:'', logoFile:null })
  ...
  async function handleGenerate(e?:any){
    ...
    try{
      ...
      setStep(2)
    }catch(err){ console.error(err); alert('Genereren mislukt') }
    setLoading(false)
  }
